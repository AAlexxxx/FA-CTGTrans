...
def interpolation(base_line, the_count):
    ...
    while np.count_nonzero(base_line == 0) > 0:  # 寻找需要插值的位置
...
def bad_value_processing(input):
    The_length_window = 500  # 移动窗长度100
    bad_length = len(input)  # 数据总长度
    bad_length = (
                             bad_length // The_length_window) * The_length_window  # 确保数据不溢出，向下取整，假如数据长度999，ceil（999/100）= 9 * 100 = 900 ,仅处理前900数据，为了防止溢出，后99个数据不处理
    for ii in range(0, bad_length, The_length_window):
        mean_signal = sum(input[ii:ii + The_length_window]) / The_length_window  # 取窗内平均值
        std_signal = np.std(input[ii:ii + The_length_window])  # 取窗内标准差
        Bad_value = np.where((input[ii:ii + The_length_window] > (mean_signal + 3 * std_signal)) | (
                    input[ii:ii + The_length_window] < (mean_signal - 3 * std_signal)))  # 拉以达法则
        input[Bad_value[0] + ii - 1] = 0  # 若存在坏值，坏值置零
    mean_input = np.mean(input)
    output = interpolation(input, mean_input)  # 插值
    return output
