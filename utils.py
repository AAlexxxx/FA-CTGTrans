...
def _save_metrics_val(pred_labels, true_labels, log_dir, home_path, classes_names):
    pred_labels = np.array(pred_labels).astype(int)
    true_labels = np.array(true_labels).astype(int)
    ...
    # save classification report
    file_name = "classification_report_val.xlsx"
    report_Save_path = os.path.join(home_path, log_dir, file_name)
    df.to_excel(report_Save_path)


def _save_metrics_test(pred_labels, true_labels, log_dir, home_path, classes_names):
    pred_labels = np.array(pred_labels).astype(int)
    true_labels = np.array(true_labels).astype(int)
    ...
    # save classification report
    file_name = "classification_report_test.xlsx"
    report_Save_path = os.path.join(home_path, log_dir, file_name)
    df.to_excel(report_Save_path)
